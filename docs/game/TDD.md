...
> 文档版本：0.4<br>
> 状态：已同步 PRD 0.4 与 DEC-012 的 macOS arm64 发行与验收范围；可作为 M0–M3 实施基线<br>
> 专业起草角色：🎮 `gameplay`<br>
> 技术质量负责人：🧱 `tech`<br>
> 独立可测性审查：🛡️ `quality`<br>
> 产品依据：[PRD.md](PRD.md)

> DEC-013 运行端覆盖：P0-007 及其后的活跃迁移任务使用 Xcode + Swift + SpriteKit、Codable JSON 与 XCTest。本文尚未迁移的 Godot/GDScript 专属段落只作为历史架构与行为参考；M1-002 的权威增量合同见 [m1-002-brief.md](m1-002-brief.md)。
...
| 项目 | 决策 |
|---|---|
| 引擎 | Godot 4.7.1 标准版（非 .NET）；具体版本已锁定，未经 Product Owner 批准不得升级 |
| 语言 | 强类型 GDScript |
...
具体 Godot 小版本已写入 `project.godot` 和[构建与引擎基线](build.md)，团队不得在开发中静默升级。`project.godot` 的 `config/engine_version_lock` 保存精确补丁版本；升级前必须遵循构建说明中的 macOS arm64 兼容与回归验证策略。
...
M1/VS-0 的冻结基线为每个现实秒推进 `1.3` 个游戏分钟；实现应保存小数余量，只提交完整游戏分钟，并在 23:30 截止。
...
M1 的 `hoe`、`seed`、`water` 与 `harvest` 每次成功提交固定消耗 `2` 点体力；验证失败时体力、库存和地块必须全部保持原值。砍伐、采掘等后续动作不属于 M1-002，继续由数据定义其 `2–5` 点成本。
...
| 平台 | 最低参考配置 | 导出 |
|---|---|---|
| macOS | 13 英寸 MacBook Air（M4，2025；Mac16,12 / MW0W3CH/A）；macOS 26.5.2、Apple M4（10 核 CPU / 8 核 GPU）、16 GB、内置 256 GB SSD、60 Hz | arm64 |

该 MacBook Air 是当前性能基线。设备不可用时，质量报告必须标记 `BLOCKED` 或记录经基准证明不高于参考性能的替代设备。所有性能报告记录实际 CPU、GPU、内存、系统、Godot 版本、构建类型和显示刷新率。

测试方法：在 macOS arm64 发布导出中使用 Compatibility 渲染器和 1920×1080；预热 60 秒后运行 10 分钟自动压力路线，覆盖两张地图、7 名 NPC、雨天、最大切片农田、制作放置、社交事件和一次存档。
...
| 动态放置对象 | 单地图 500 以内 |
| 农田逻辑格 | 单地图 4,096 以内 |
| 地图常规加载 | macOS arm64 连续切图 10 次，P95 <3 秒 |
| 自动存档 | macOS arm64 执行 20 次，P95 <500 ms；必须提供保存中反馈 |
| 内存 | 开发构建记录峰值，垂直切片目标低于 1 GB |

达到预算 80% 时开始分析，不等到超出预算才优化。

macOS arm64 的每个候选版本都必须在断网环境完成安装/启动、新游戏、键鼠或手柄输入、待结算出售保存读取和 VS-0 烟雾路径。
...
PRD 第 14 节中的“未经核实便转述”“暂缓后过期”和“核实后纠正”是互斥路径，质量流程必须使用三个从同一事件前状态派生的独立存档夹具执行，不能要求同一事件实例连续完成多个结果。性能报告必须附 macOS arm64 的 10 分钟采样；发行烟雾证据必须包含断网启动。
...
| PRD-NFR-001 | 性能预算、Profiler | macOS arm64 指定硬件档 10 分钟 P95/P99 帧时间记录 |
| PRD-NFR-002 | SceneRouter、资源加载 | macOS arm64 连续 10 次切图 P95 计时 |
...
| PRD-NFR-011 | 导出预设、发行烟雾 | macOS arm64 断网安装、启动和完整 VS-0 测试 |
...
技术门禁：无内容或必需输入错误；新建、保存、读取空会话通过；macOS arm64 空工程可断网启动。
...
- 两张地图、美术音频替换、设置和手柄通路。
- macOS arm64 导出、性能、回归、设置/存档失败恢复和完整烟雾测试。

质量门禁：PRD 第 10、11、14 节和 NFR-001、002、010、011 全部有 macOS arm64 证据。
...
| macOS 单平台开发掩盖目标设备性能问题 | M0 建立 macOS arm64 预设，M4 候选版执行指定硬件档、断网和完整烟雾门禁 |
...
- 🛡️ Quality & Delivery：`APPROVED`，首笔出售实际到账，三个互斥社交路径、边界夹具、macOS 性能与保存读取均有独立证据要求。
