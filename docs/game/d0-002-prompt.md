# D0-002 Cursor 开工 Prompt（粘贴给 Cursor / Codex）

> 本文件是 `docs/game/d0-002-brief.md` 的执行版。Cursor 是唯一实现负责人；完成后经 🧱 tech 交付门禁与 🛡️ quality 独立验收，任何人不得自行 APPROVED。
...
- 契约依据：PRD 6.12（存档至少包含…设置状态）、6.13（机器设置文件/重绑定/无障碍）、FR-010、NFR-010（见 `docs/game/PRD.md`）；任务合同 `docs/game/d0-002-brief.md`。
...
## 6. 交付格式（全部放入 `artifacts/integration/d0-002-xcode/`）
