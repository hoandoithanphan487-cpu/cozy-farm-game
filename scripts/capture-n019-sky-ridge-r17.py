#!/usr/bin/env python3
"""Capture N-019 R17 sharp north-bleed sky-and-ridge proof in the real App."""
...
APP = Path(os.environ.get(
    "N019_R17_APP",
    "/tmp/creeksprout-n019-r17-dd/Build/Products/Debug/CreekSprout.app",
))
OUT = ROOT / "artifacts/integration/n-019-demo-visual/sky-ridge-r17/real-app"

helper_path = ROOT / "scripts/drive-n017-demo-app.py"
spec = importlib.util.spec_from_file_location("n019_r17_driver", helper_path)
...
def main() -> None:
    ...
    driver.LOG.write_text(
        "# N-019 R17 sharp north-bleed sky ridge real App capture\n\n",
        encoding="utf-8",
    )
    ...
    driver.walk_to(7, 10)
    driver.capture(
        "01-farm-north-sky-ridge-r17",
        "农场：山与蓝天从 y=14 顶部带起，保持在所有房屋之后；原生 1px 细节",
    )
    ...
    driver.walk_to(9, 10)
    driver.capture(
        "02-market-north-sky-ridge-r17",
        "集市：山与蓝天只在 y=14 以北的三格外溢带；建筑、人物、花田位置未动",
    )
